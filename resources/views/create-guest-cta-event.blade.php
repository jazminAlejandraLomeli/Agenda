<x-app-layout>
    <x-slot name="header">
        <div class="flex dark:text-white items-center gap-3">
            ...
            <h2 class="ml-2 md:text-2xl text-xl font-bold">Reservar aula</h2>
        </div>
    </x-slot>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-lg max-w-7xl p-5 md:p-10 mx-auto mt-5 rounded">
        ...
        <form class="grid grid-cols-1 md:grid-cols-2 grid-flow-row" action="{{ route('agenda.guest.classrom.store',['type_events' => 'classrooms']) }}" method="POST" id="form-create-event">
            @csrf
            <div  class="col-span-1 md:col-span-1 md:me-5">
                <x-group-form>
                    <x-slot name="label">Nombre de la reservación</x-slot>
    ...
    @vite(['resources/js/create-guest-event-cta.js'])
