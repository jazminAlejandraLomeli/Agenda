<x-app-layout>
    <x-slot name="header">
        <div class="flex dark:text-white items-center gap-3">
            ...
            <h2 class="ml-2 md:text-2xl text-xl font-bold">Reservar laboratorio</h2>
        </div>
    </x-slot>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-lg max-w-7xl p-5 md:p-10 mx-auto mt-5 rounded">
        ...
        <form class="grid grid-cols-1 md:grid-cols-2 grid-flow-row" action="{{ route('agenda.guest.classrom.store',['type_events' => 'laboratory']) }}" method="POST" id="form-create-event">
            @csrf
            ...
            @include('layouts.date-repetition-only-day')
    ...
    @vite(['resources/js/create-guest-event-laboratory.js'])
