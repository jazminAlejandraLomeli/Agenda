<x-app-layout>
    <x-slot name="header">
        <div class="flex dark:text-white items-center gap-3">
            ...
            <h2 class="ml-2 text-2xl font-bold">Editar evento</h2>
        </div>
    </x-slot>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-lg max-w-7xl p-10 mx-auto mt-5 rounded">
        ...
        {{-- {{(isset($recursive_event) && $recursive_event)}} --}}

        <form class="grid grid-cols-1 md:grid-cols-2"
            action="{{ (isset($recursive_event) && $recursive_event) ? route('agenda.update.cta.all', ['id' => $event->id]) : route('agenda.update.cta', ['id' => $event->id]) }}"
            ...
            <div class="me-5">
                ...
                <x-group-form>
                    ...
                    <x-input-primary type="email" name="email" value="{{ $event->cta->email }}"></x-input-primary>
                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" class="mx-2 text-primary"
                        viewBox="0 0 24 24">
                        <path fill="currentColor"
                    ...
                    <x-select-primary name="event_type" id="eventType">
                        ...
                        @if (!$events_types->isEmpty())
                            @foreach ($events_types as $type)
                                @if ($type->id == $event->event_type->id)
            ...
            <div class="ms-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-group-form class="cols-span-1">
                        ...
                        <x-input-primary type="number" value="{{ $event->cta->num_participants }}"
                            name="num_participants"></x-input-primary>
                    </x-group-form>
                        ...
                        <x-select-primary name="semester">
                            ...
                            @if (!$semesters->isEmpty())
                                @foreach ($semesters as $semester)
                                    @if ($semester->id == $event->cta->semester->id)
                ...
                @else
                    <x-only-date :date="$event->date->date_start" :hourstart="$event->date->only_hour_start" :hourend="$event->date->only_hour_end" />
                @endif
    ...
    @if (isset($recursive_event) && $recursive_event)
        @vite('resources/js/edit-events-cta.js');

    @else
        @vite('resources/js/edit-event-cta.js');
    @endif
