<x-app-layout>
    <x-slot name="header">
        <div class="flex dark:text-white items-center gap-4">
            <x-tertiary-link-button href="{{ route('users.index') }}">                
                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 24 24">
                        <path fill="currentColor"
                            d="m9.55 12l7.35 7.35q.375.375.363.875t-.388.875t-.875.375t-.875-.375l-7.7-7.675q-.3-.3-.45-.675t-.15-.75t.15-.75t.45-.675l7.7-7.7q.375-.375.888-.363t.887.388t.375.875t-.375.875z" />
                    </svg>                
            </x-tertiary-link-button>
            <h2 class="text-2xl font-bold">Agregar usuario</h2>
        </div>
    </x-slot>

    <div class="bg-white dark:bg-gray-800 shadow max-w-7xl p-10 mx-auto mt-5 rounded">



        @if ($errors->any())
            <x-list-alert type="danger" id="listAlert">
                <x-slot name="title">¡Ups!</x-slot>
                <x-slot name="subtitle">Algo salió mal.</x-slot>
                @foreach ($errors->all() as $error)
                    <li>{{ $error }}</li>
                @endforeach
            </x-list-alert>
        @endif

        @if (session('error'))
            <x-alert type="danger">
                <x-slot name="title">¡Ups!</x-slot>
                {{ session('error') }}
            </x-alert>
        @endif

        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">

            <div class="flex flex-col justify-start  border-e-2 pe-5">
                <x-group-form class="me-3 w-full">
                    <x-slot name="label">Código</x-slot>
                    <x-input-primary type="text" name="code" id="code"></x-input-primary>
                </x-group-form>
                <div>
                    <x-secondary-button id="search-code">
                        Buscar
                    </x-secondary-button>
                </div>
            </div>

            <div>
                <div id="instructions">
                    <p class="text-gray-600 dark:text-gray-100 text-md font-semibold">Al buscar el código el nombre de la persona aparecerá en este espacio.</p>
                </div>
                <form action="{{ route('users.create.store') }}" method="POST"
                    class="hidden animate__animated animate__fadeIn " id="form">
                    @csrf
                    <x-group-form>
                        <x-slot name="label">Nombre</x-slot>
                        <x-input-primary type="text" name="name" id="name"></x-input-primary>
                    </x-group-form>

                    <x-group-form>
                        <x-slot name="label">Nombre de usuario</x-slot>
                        <x-input-primary type="text" name="user_name"></x-input-primary>
                    </x-group-form>

                    <x-group-form>
                        <x-slot name="label">Grupo</x-slot>
                        <x-select-primary name="group" id="group">
                            <option selected value="" disabled>Selecciona un grupo</option>
                            @foreach ($groups as $group)
                                <option value="{{ $group->id }}">{{ strtoupper($group->type) }}</option>
                            @endforeach
                        </x-select-primary>
                    </x-group-form>

                    <x-group-form>
                        <x-slot name="label">Rol</x-slot>
                        <x-select-primary name="role" id="role">
                            <option value="" selected disabled>Selecciona un rol</option>
                            @foreach ($roles as $role)
                                <option value="{{ $role->id }}">{{ strtoupper($role->name) }}</option>
                            @endforeach
                        </x-select-primary>
                    </x-group-form>

                    <hr class="mb-4" />

                    <x-alert-permission />



                    <div id="permissions"></div>


                    <x-accordion title="Lista de permisos" class="accordionPermission">
                        <ul id="listPermission" class="m-3"></ul>
                    </x-accordion>

                    <span class="text-red-500 pt-1 border-t-2 hidden" id="error-permissions"></span>

                    <x-group-form>
                        <x-slot name="label">Permisos sin usar</x-slot>
                        <x-select-primary name="unassigned_permissions" id="unassigned-permissions">
                            <option selected disabled>Selecciona el permiso</option>
                        </x-select-primary>
                    </x-group-form>

                    <hr class="mb-4" />

                    <p class="dark:text-white text-lg font-light">La contraseña por defecto del usuario será - "<span
                            class="font-bold">{{ $defaultPassword }}</span>" - , esta se debe cambiar lo antes posible.
                    </p>

                    <div class="flex justify-end gap-5 mt-5">
                        <x-tertiary-button>
                            Cancelar
                            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" class="ms-3"
                                viewBox="0 0 24 24">
                                <path fill="currentColor"
                                    d="m12 13.4l2.9 2.9q.275.275.7.275t.7-.275t.275-.7t-.275-.7L13.4 12l2.9-2.9q.275-.275.275-.7t-.275-.7t-.7-.275t-.7.275L12 10.6L9.1 7.7q-.275-.275-.7-.275t-.7.275t-.275.7t.275.7l2.9 2.9l-2.9 2.9q-.275.275-.275.7t.275.7t.7.275t.7-.275zm0 8.6q-2.075 0-3.9-.788t-3.175-2.137T2.788 15.9T2 12t.788-3.9t2.137-3.175T8.1 2.788T12 2t3.9.788t3.175 2.137T21.213 8.1T22 12t-.788 3.9t-2.137 3.175t-3.175 2.138T12 22m0-2q3.35 0 5.675-2.325T20 12t-2.325-5.675T12 4T6.325 6.325T4 12t2.325 5.675T12 20m0-8" />
                            </svg>
                        </x-tertiary-button>

                        <x-secondary-button type="submit">
                            Guardar
                            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" class="ms-3"
                                viewBox="0 0 24 24">
                                <path fill="none" stroke="currentColor" stroke-linecap="round"
                                    stroke-linejoin="round" stroke-width="2"
                                    d="M15 11h3m0 0h3m-3 0v3m0-3V8m-3 11v-1.25c0-2.071-1.919-3.75-4.286-3.75H7.286C4.919 14 3 15.679 3 17.75V19m9-11a3 3 0 1 1-6 0a3 3 0 0 1 6 0" />
                            </svg>
                        </x-secondary-button>
                    </div>
                </form>
            </div>

        </div>

    </div>

    {{-- @include('./layouts/toast') --}}

    @vite('resources/js/create-user.js');

</x-app-layout>
