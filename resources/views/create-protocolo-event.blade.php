...
<x-app-layout>
    <x-slot name="header">
        <div class="flex dark:text-white items-center gap-3">
            ...
            <h2 class="ml-2 text-xl md:text-2xl font-bold">Agregar evento</h2>
        </div>
    </x-slot>
    ...
    {{-- <div class="w-10 h-10 custom-option">
        Hola
    </div> --}}


    <div class="bg-white dark:bg-gray-800 shadow max-w-7xl p-5 md:p-10 mx-auto mt-5 rounded">
        ...
        <form class="grid grid-cols-1 md:grid-cols-2 grid-flow-row" action="{{ route('agenda.store.protocolo') }}" method="POST"
            id="form-create-event">
            @csrf
            <div class="md:me-5 col-span-1 md:col-span-1 ">

                <input type="hidden" name="group_id" id="group-id" value="{{ $group->id }}">

                {{-- @include('layouts.handle-title-event') --}}

                <x-group-form class="w-full">
                    <x-slot name="label">Nombre del evento</x-slot>
                    <x-input-primary type="text" id="titleEvent" name="title" value="{{ old('title') }}" placeholder="Agregar o elegir un título"></x-input-primary>
                ...
                <x-group-form>
                    ...
                    <x-input-primary maxlength="10" name="phone" value="{{ old('phone') }}"></x-input-primary>
            ...
            <div class="md:ms-5 col-span-1 md:col-span-1">                
                ...
                <div class="mt-5">
                    <x-group-form>
                        ...
                        <x-text-primary name="notes_protocolo">{{$notesDefault ?? 'Sin notas'}}</x-text-primary>

                    </x-group-form>

                    <x-group-form>
                        <x-slot name="label">Notas para CTA</x-slot>
                        <x-text-primary name="notes_cta">{{$notesDefault ?? 'Sin notas'}}</x-text-primary>

                    </x-group-form>

                    <x-group-form>
                        <x-slot name="label">Notas para Servicios Generales</x-slot>
                        <x-text-primary name="notes_general_services">{{$notesDefault ?? 'Sin notas'}}</x-text-primary>
                    </x-group-form>
                </div>

            </div>

            <div class="md:col-span-2  flex justify-end items-start gap-5 mt-3">
    ...
    @vite('resources/js/create-event-protocolo.js')
